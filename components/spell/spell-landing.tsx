...
const principles = [
  {
    icon: Lock,
    titleKey: "spell.principle.1.title",
    descKey: "spell.principle.1.desc",
  },
  {
    icon: Webhook,
    titleKey: "spell.principle.2.title",
    descKey: "spell.principle.2.desc",
  },
  {
    icon: RefreshCw,
    titleKey: "spell.principle.3.title",
    descKey: "spell.principle.3.desc",
  },
  {
    icon: Key,
    titleKey: "spell.principle.4.title",
    descKey: "spell.principle.4.desc",
  },
  {
    icon: FileText,
    titleKey: "spell.principle.5.title",
    descKey: "spell.principle.5.desc",
  },
  {
    icon: Shield,
    titleKey: "spell.principle.6.title",
    descKey: "spell.principle.6.desc",
  },
]
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b border-border py-16">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <h1 className="mb-8 text-4xl font-light tracking-tight text-foreground md:text-5xl text-balance">
            {t("spell.landing.headline")}
          </h1>
          <div className="mx-auto max-w-2xl space-y-2 text-muted-foreground">
            <p>{t("spell.landing.desc1")}</p>
            <p>{t("spell.landing.desc2")}</p>
            <p>{t("spell.landing.desc3")}</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          <div className="rounded-lg border border-m-spell-primary/30 bg-spell-primary/5 p-6">
            <h2 className="mb-4 flex items-center gap-2 text-lg font-medium text-foreground">
              <Zap className="h-5 w-5 text-spell-primary" />
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-medium text-spell-primary">
                  {language === "ja" ? "Spellという売り方" : "The Spell Way"}
                </h4>
                <ul className="space-y-2 text-sm">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-spell-primary" />
            ...
            <div className="mt-4 rounded border border-border bg-background p-3">
              <p className="text-sm font-medium text-foreground">
                {language === "ja" 
                  ? "Spell = 「この実行は許可されているか？」にYes/Noを返す判定装置"
                  : "Spell = A judgment device that returns Yes/No to 'is this execution allowed?'"}
              </p>
              ...
              <p className="mt-2 text-xs text-muted-foreground border-t border-border pt-2">
                {language === "ja"
                  ? "第三者が自分のSpellを作りたい場合: 自分でデプロイし、Spell APIに接続する"
                  : "If third parties want their own Spells: Deploy themselves and connect to the Spell API"}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={item.step} className="relative rounded-lg border border-border bg-card p-4">
                <div className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-spell-primary text-xs font-bold text-white">
                  {item.step}
                </div>
                <item.icon className="mb-2 h-5 w-5 text-spell-primary" />
          ...
          <p className="mt-4 text-center text-xs text-muted-foreground">
            {language === "ja"
              ? "※ 購入者が「Spellの管理画面」を触る必要は基本ない"
              : "* Buyers generally never need to touch Spell's dashboard"}
          </p>
        </div>
      </section>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="mb-3 flex items-center gap-2">
                <Code className="h-5 w-5 text-spell-primary" />
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                ...
                <p className="text-muted-foreground/60">↓</p>
                <p className="text-spell-primary">{language === "ja" ? "通る" : "Access granted"}</p>
              </div>
            </div>
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="mb-3 flex items-center gap-2">
                <Terminal className="h-5 w-5 text-spell-primary" />
                <h3 className="font-medium text-foreground">
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                ...
                <p className="text-muted-foreground/60">↓</p>
                <p className="text-spell-primary">{language === "ja" ? "再実行→通る" : "Re-run → Success"}</p>
              </div>
            </div>
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="mb-3 flex items-center gap-2">
                <Server className="h-5 w-5 text-spell-primary" />
                <h3 className="font-medium text-foreground">
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                ...
                <p className="text-muted-foreground/60">↓</p>
                <p>OK: <span className="text-spell-primary">presigned URL</span></p>
...
await runMyExpensiveOperation()`}</code>
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            {language === "ja"
              ? "Spellは判定だけ。実行・保存・配信はあなたのサーバーで。"
              : "Spell only judges. Execution, storage, delivery is on your server."}
          </p>
        </div>
      </section>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="rounded border border-border bg-muted/30 p-4">
                <div className="mb-3 flex items-center gap-2">
                  <Unlock className="h-5 w-5 text-spell-primary" />
                  <span className="text-sm font-mono text-muted-foreground">
                    entitled = true
                  </span>
                </div>
                <div className="space-y-2 font-mono text-sm">
                  <div className="flex items-center gap-2 text-spell-primary">
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          <h2 className="mb-8 text-center text-xl font-medium text-foreground">
            {t("spell.landing.problems_title")}
          </h2>
              ...
              <tbody>
                {problems.map((row, i) => (
                  <tr key={i} className="border-b border-border">
                    ...
                    <td className="px-4 py-3 font-mono text-spell-primary">
                      {row.solution}
                    </td>
...
[ Audit Log / Ledger ]    ← append-only, reconcilable`}
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          <h2 className="mb-8 text-center text-xl font-medium text-foreground">
            {t("spell.landing.how_it_works")}
          </h2>
          <div className="grid gap-4 md:grid-cols-4">
            <div className="rounded-lg border border-border bg-card p-4 text-center">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-spell-primary/10 text-spell-primary">
                <span className="text-lg font-bold">1</span>
              </div>
              <h3 className="mb-1 font-medium text-foreground">
                {t("spell.landing.step1_title")}
              </h3>
              <p className="text-sm text-muted-foreground">
                {t("spell.landing.step1_desc")}
              </p>
            </div>
            <div className="rounded-lg border border-border bg-card p-4 text-center">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-spell-primary/10 text-spell-primary">
              ...
              <h3 className="mb-1 font-medium text-foreground">
                {t("spell.landing.step2_title")}
              </h3>
              <p className="text-sm text-muted-foreground">
                {t("spell.landing.step2_desc")}
              </p>
            </div>
            <div className="rounded-lg border border-border bg-card p-4 text-center">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-spell-primary/10 text-spell-primary">
              ...
              <h3 className="mb-1 font-medium text-foreground">
                {t("spell.landing.step3_title")}
              </h3>
              <p className="text-sm text-muted-foreground">
                {t("spell.landing.step3_desc")}
              </p>
            </div>
            <div className="rounded-lg border border-border bg-card p-4 text-center">
              <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-spell-primary/10 text-spell-primary">
              ...
              <h3 className="mb-1 font-medium text-foreground">
                {t("spell.landing.step4_title")}
              </h3>
              <p className="text-sm text-muted-foreground">
                {t("spell.landing.step4_desc")}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          <h2 className="mb-8 text-center text-xl font-medium text-foreground">
            {t("spell.landing.features_title")}
          </h2>
          <div className="grid gap-3 md:grid-cols-5">
            {[
              ...
              return (
                ...
                >
                  <Icon className="mx-auto mb-2 h-6 w-6 text-spell-primary" />
                  <p className="text-sm font-medium text-foreground">
                    {feature.label}
                  </p>
      ...
      <section className="border-b border-border py-12">
        <div className="mx-auto max-w-4xl px-4">
          <h2 className="mb-8 text-center text-xl font-medium text-foreground">
            {t("spell.landing.user_flow_title")}
          </h2>
          <div className="rounded-lg border border-border bg-card p-6">
                ...
                <div key={i} className="flex items-center gap-2">
                  <span
                    className={`rounded px-2 py-1 ${
                      i === 5
                        ? "bg-spell-primary/20 text-spell-primary"
                        : i === 8
