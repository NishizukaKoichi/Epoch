...
const codeExamples = {
  runtime: {
    ...
    code: `// Spell Runtime - 完全実装
// 
// これは何か:
// - 処理を実行するランタイムではない
// - 処理の実行前に、必ずSpellに実行可否を問い合わせさせる枠組み
// - 処理は第三者が作る、環境も第三者が持つ、Runtimeは「勝手に動かない殻」
//
// Runtimeがやること（これだけ）:
// 1. Spell に Scope Check を送る
// 2. 返答を待つ
...
export function createRuntime(config: RuntimeConfig) {
  const endpoint = config.endpoint || 'https://api.spell.dev/v1/check'
...
// 
// 支配点はRuntime側ではなく
// Spell側の Entitlement と API Key
// 
// Keyを失効させれば、
// 世界中にデプロイされたRuntimeが同時に沈黙する`,
  },
  talisman: {
    title: "Talisman + Spell",
    description: "認証（Talisman）→ 認可（Spell）の基本パターン",
    code: `// Talisman = 「誰か」を確定（認証）
// Spell = 「実行していいか」を判定（認可 + 課金）
...
export async function runPremiumFeature(credential: string) {
  ...
  // 2. Spellで権利を判定
  const { entitled } = await spell.check({
...
// Talismanが壊れても → Spellの判定ロジックは無傷
// Spellが壊れても → Talismanの認証は無傷
// それぞれ独立して動く`,
  },
  check: {
...
const response = await fetch('https://api.spell.dev/v1/check', {
  method: 'POST',
  headers: {
...
export async function POST(req: Request) {
  ...
  // Spellにイベントを転送
  await fetch('https://api.spell.dev/v1/webhook/stripe', {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer YOUR_API_KEY',
      'Content-Type': 'application/json'
...
import { checkEntitlement } from './spell';
...
async function checkEntitlement(userId: string, scope: string) {
  const res = await fetch('https://api.spell.dev/v1/check', {
    method: 'POST',
    headers: {
...
// 1. 付与
await fetch('https://api.spell.dev/v1/entitlements', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer YOUR_API_KEY' },
  body: JSON.stringify({
    user_id: 'user_abc123',
...
// 2. 剥奪
await fetch('https://api.spell.dev/v1/entitlements/ent_xyz', {
  method: 'DELETE',
  headers: { 'Authorization': 'Bearer YOUR_API_KEY' },
  body: JSON.stringify({
    reason: '返金対応 #5678'
...
await fetch('https://api.spell.dev/v1/reconcile', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer YOUR_API_KEY' },
  body: JSON.stringify({
    user_id: 'user_abc123',
...
// - SpellのEntitlementと突合
// - 差分があれば修正
// - Audit Logに記録`,
  },
}

export function SpellIntegration() {
  const { t } = useI18n()
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          あなたのプロダクトにSpellを統合する方法
        </p>
      </div>

      {/* Key Concept */}
      <div className="rounded-lg border border-spell-primary/30 bg-spell-primary/5 p-6 mb-8">
        <h2 className="font-medium text-foreground mb-3">Talisman + Spell</h2>
        <div className="grid gap-4 md:grid-cols-2 mb-4">
          ...
          <div className="rounded border border-border bg-background p-3">
            <div className="text-sm font-medium text-spell-primary mb-1">Spell</div>
            <p className="text-xs text-muted-foreground">
        ...
        <div className="text-xs text-muted-foreground">
          ...
          <span className="font-mono bg-muted px-1 ml-1">Spell.check(userId, scope)</span> → entitled
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          <div className="rounded-lg border border-border bg-card p-4 flex items-start gap-4">
            <div className="rounded-full bg-spell-primary/20 w-8 h-8 flex items-center justify-center text-sm font-mono text-spell-primary shrink-0">1</div>
            <div>
              <h3 className="font-medium text-foreground mb-1">商品とScopeを定義</h3>
              <p className="text-sm text-muted-foreground">
                Spellで商品を登録し、各商品にScopeを紐づける。
                Scopeは「pro:execute」「cli:download」のような識別子。
              </p>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-4 flex items-start gap-4">
            <div className="rounded-full bg-spell-primary/20 w-8 h-8 flex items-center justify-center text-sm font-mono text-spell-primary shrink-0">2</div>
            <div>
              <h3 className="font-medium text-foreground mb-1">Stripe Webhookを設定</h3>
              <p className="text-sm text-muted-foreground">
                StripeのWebhookをSpellに転送するエンドポイントを作成。
                checkout.session.completedでEntitlementが自動付与される。
              </p>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-4 flex items-start gap-4">
            <div className="rounded-full bg-spell-primary/20 w-8 h-8 flex items-center justify-center text-sm font-mono text-spell-primary shrink-0">3</div>
            <div>
        ...
        <Tabs defaultValue="runtime" className="w-full">
          <div className="border-b border-border px-4">
            <TabsList className="h-12 bg-transparent gap-4 flex-wrap">
              <TabsTrigger value="runtime" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Runtime
              </TabsTrigger>
              <TabsTrigger value="talisman" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Talisman連携
              </TabsTrigger>
              <TabsTrigger value="check" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Check
              </TabsTrigger>
              <TabsTrigger value="webhook" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Webhook
              </TabsTrigger>
              <TabsTrigger value="download" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Download
              </TabsTrigger>
              <TabsTrigger value="cli" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                CLI
              </TabsTrigger>
              <TabsTrigger value="reconcile" className="data-[state=active]:bg-transparent data-[state=active]:text-spell-primary data-[state=active]:shadow-none">
                Reconcile
              </TabsTrigger>
            </TabsList>
          </div>
      ...
      <div className="mt-6 flex items-center justify-end gap-4">
        <a
          href="#"
          className="text-sm text-spell-primary hover:underline flex items-center gap-1"
        >
          API Reference
