...
import { ChevronLeft, Shield, Fingerprint, Mail, Phone, Globe, Key, Check, ArrowRight, Loader2 } from "@/components/icons"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { useI18n } from "@/lib/i18n/context"
import { useAuth, type Credential } from "@/lib/auth/context"
...
export default function RegisterPage() {
  ...
  const handleVerify = async () => {
    ...
    // 登録完了: 使用した認証手段をそのまま1つ目のCredentialとして追加
    const newCredential: Credential = {
      id: `cred-${Date.now()}`,
      type: selectedMethod.type,
      label: selectedMethod.type === "passkey" 
        ? "このデバイス" 
        : inputValue,
                  ...
                  {selectedMethod.type === "oauth" && (
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {["Google", "GitHub", "Apple", "X"].map((provider) => (
                          <Button
                            key={provider}
                            variant="outline"
                            className="bg-transparent"
                            onClick={() => setInputValue(provider)}
                          >
                            {provider}
                          </Button>
                        ))}
