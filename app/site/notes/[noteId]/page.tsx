...
この不可逆性こそが、Epochの価値の源泉だ。
    ...
    tags: ["Epoch", "設計思想"],
    relatedLinks: [
      { title: "Epoch Spec", href: "/landing" },
      { title: "Epoch MVP", href: "/" },
    ],
...
これが「プロダクト間の境界線」の一例だ。
    ...
    tags: ["Sigil", "設計思想"],
    relatedLinks: [
      { title: "Sigil Library", href: "/site/library/sigil" },
    ],
...
何をしないかを決めることは、何をするかを決めることと同じくらい重要だ。
    ...
    tags: ["全体設計"],
    relatedLinks: [
      { title: "Library", href: "/site/library" },
    ],
  ...
  if (!note) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-16">
        ...
        <Link href="/site/notes" className="text-sm text-foreground hover:underline mt-4 inline-block">
          Back to Notes
        </Link>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16">
      ...
      <nav className="mt-12 pt-8 border-t border-border flex items-center justify-between">
        {note.prevNote ? (
          <Link
            href={`/site/notes/${note.prevNote.id}`}
        ...
        {note.nextNote ? (
          <Link
            href={`/site/notes/${note.nextNote.id}`}
            className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
