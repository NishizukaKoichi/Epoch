...
**Spell Runtime は、処理を実行するためのランタイムではない。**  
**処理の実行前に、必ず Spell に実行可否を問い合わせさせるための実行枠組み**。
...
・Spell に接続できない場合は fail-closed  
・Scope Check をスキップする経路が存在しない

言い換えると、  
**Spell を通らずに処理が走る経路が一つも無いこと**。
...
支配点は Runtime ではなく  
**Spell 側の Entitlement と API Key**。
...
以下が成立していれば、Spell Runtime は完成。

・第三者が任意の処理を組み込める  
・第三者が自分の環境にデプロイできる  
・実行前に必ず Scope Check が走る  
・Yes / No 以外の分岐が存在しない  
・Spell 側の変更なしで Spell を増やせる
